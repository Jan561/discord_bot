...
#[command]
async fn link(ctx: &Context, msg: &SerenityMessage, args: Args) -> CommandResult {
    match _link(ctx, msg, args).await {
        Ok(()) => Ok(()),
        Err(why) => match why {
            GeneralError::ExecutionError(err) => {
                Err(err)?;

                unreachable!();
            }
            GeneralError::CommandError(err) => {
                println!("{:?}", err);
...
async fn _link(ctx: &Context, msg: &SerenityMessage, mut args: Args) -> Result<(), GeneralError> {
